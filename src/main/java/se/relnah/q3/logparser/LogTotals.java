...
public class LogTotals {
    ...
    /*
     * 
     * This class is built to hold user totals for ease of use. It can work out
     * the average efficiency, rank and kill ratio, as well as holding weapon
     * info.
     * 
    ...
    // Can be called with the initial number of users (used for individual user
    // totals)
    //
    ...
    //
    //
    public void addSuicide(int iNum) {
        fSuicides += iNum;
    }
    ...
    //
    //
    public void addDeath(int iNum) {
        fDeaths += iNum;
    }

    // Adds a user to the totals (either a valid one for the totals or one that
    // doesnt count)
    //
    // Written by : Stuart Butcher
    //
    ...
    //
    public int getFrags() {
        return new Float(fFrags).intValue();
    }
    ...
    public int getKills(boolean bDivide) {
        if (bDivide) {
            return new Float(fKills / fUsers).intValue();
        } else {
            return new Float(fKills).intValue();
        }
    }

    public int getKills() {
        return getKills(true);
    }
    ...
    public int getActualDeaths() {
        return new Float(fDeaths).intValue();
    }
    ...
    //
    //
    public int getActualSuicides() {
        return new Float(fSuicides).intValue();
    }
    ...
    public int getSuicides() {
        return new Float(fSuicides / fUsers).intValue();
    }
    ...
    public int getDeaths() {
        return new Float(fDeaths / fUsers).intValue();
    }
    ...
    //
    public int getUsers() {
        return new Float(fUsers).intValue();
    }
    ...
    public int getTotalUsers() {
        return new Float(fUsers + fOtherUsers).intValue();
    }
    ...
    public String getEfficiency() {
        return new Integer(efficiency(fKills, fDeaths, fSuicides).intValue()).toString();
    }
    ...
    public String getKillRatio() {
        return myTools.decimalPlaces(killratio(fKills, fDeaths, fSuicides).toString());
    }
    ...
    public String getRank() {
        return new Integer(rank(fKills, fDeaths, fSuicides).intValue()).toString();
    }
    ...
    public float getNum(int iNum) {
        float fReturn = 0;
        if (iNum == KILLS) {
            fReturn = fKills / fUsers;
        } else if (iNum == DEATHS) {
            fReturn = fDeaths / fUsers;
        } else if (iNum == SUICIDES) {
            fReturn = fSuicides / fUsers;
        } else if (iNum == RANK) {
            fReturn = rank(fKills, fDeaths, fSuicides).floatValue();
        } else if (iNum == RATIO) {
            fReturn = killratio(fKills, fDeaths, fSuicides).floatValue();
        } else if (iNum == EFFICIENCY) {
            fReturn = efficiency(fKills, fDeaths, fSuicides).floatValue();
        }
        return fReturn;
    }
        ...
        if ((fKills == 0) || (fDeaths == 0)) {
            if ((fKills == 0) && (fDeaths == 0)) {
                if (fSuicides == 0) {
                    ...
                    fReturn = new Float(-999);
                }
            } else if (fKills == 0) {
                if (fSuicides == 0) {
                    fReturn = new Float(-999);
                } else {
                    fReturn = new Float((100 / fDeaths) * (1 - (fSuicides * 2)));
                }
            } else {
                if (fSuicides == 0) {
                    fReturn = new Float(100);
                } else {
                    fReturn = new Float((100 / fKills) * (fKills - (fSuicides * 2)));
                }
            }
        } else {
            fReturn = new Float((100 / (fKills + fDeaths)) * (fKills - (fSuicides * 2)));
        }
        return fReturn;
    }
    ...
    private static Float killratio(float fKills, float fDeaths, float fSuicides) {
        ...
        if ((fKills == 0) || (fDeaths == 0)) {
            ...
            fReturn = new Float(fKills / fDeaths);
        }
    ...
    //
    //
    private static Float rank(float fKills, float fDeaths, float fSuicides) {
        return new Float(fKills - fDeaths - fSuicides);
    }
