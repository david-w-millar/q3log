...
public class LogParser {
    ...
    /*
     * 
     * This class reads log files into memory, and outputs smaller versions for
     * keeping
     * 
    ...
    private LogTools myTools = new LogTools(); // Create a LogTools object to
                                               // use
    public static final String WEAPONLINK = new String(LogTools.WEAPONLINK); // Linked
                                                                             // weapon
                                                                             // text
    // The following contains all the different lines that are defined as System
    // (feel free to add to it for other mods)
    public static final String[] TXTSYSTEM = { "InitGame", "Map", "Default pickup mode is", "arena.cfg info for map found", "ClientUserinfoChanged",
            "ClientConnect", "ClientDisconnect", "ShutdownGame", "red", "Exit", "ClientBegin", "score", "Item", "tell", "spawnhead", "touchhead",
            "AltarScore", "ClientUserinfo", "Warmup" };
    // The text preceding a chat line
    public static final String[] TXTSAY = { "say", "say_world", "say_team" };
    public static final String TXTKILL = new String("Kill"); // The text
                                                             // preceding a frag
                                                             // line
    public static final String TXTKILLED = new String("killed"); // The text
                                                                 // seperating
                                                                 // the killers
                                                                 // name from
                                                                 // the preys
                                                                 // name
    public static final String TXTGAME = new String("------------------------------------------------------------"); // New
                                                                                                                     // game
                                                                                                                     // line
    public static final String BREAK = new String(":"); // Whats after the
                                                        // commands but before
                                                        // the text in the log
                                                        // file
    ...
    public static final String TXTSUICIDE = new String("<world>"); // Used to
                                                                   // tell when
                                                                   // a kill is
                                                                   // actually a
                                                                   // suicide
    ...
    private Hashtable htFun = new Hashtable(); // Holds a list of users fun
                                               // names
    ...
    public LogParser(String sLocInFile) throws IllegalArgumentException, FileNotFoundException {
        if (myTools.checkFile(sLocInFile)) {
            fIn = new File(sLocInFile);
        }
    }
    ...
    public LogParser(String sLocInFile, String sLocOutFile) throws IllegalArgumentException, FileNotFoundException {
        ...
        if (myTools.checkString(sLocOutFile)) {
            fOut = new File(sLocOutFile);
        } else {
            throw new IllegalArgumentException("Output File Invalid");
        }
    }
    ...
    public String getOutput() {
        String sReturn = new String();
        if (fOut != null) {
            sReturn = fOut.getPath();
        }
        return sReturn;
    }

    // Parses a log file and outputs a compressed version containing just the
    // frag lines
    //
    // Written by : Stuart Butcher
    //
        ...
        if ((fIn != null) && (myTools.checkFile(fIn.getPath()))) {
            if (fOut != null) {
                ...
                if (sLine != null) {
                    ...
                    while (sLine != null) {
                        if (!sLine.equals("")) {
                            if ("vanilla".equalsIgnoreCase(mod)) {
                                sLine = sLine.substring(indent);
                            } else if ("cpma".equalsIgnoreCase(mod)) {
    ...
    public Hashtable execute(String sLocOutFile, String sLocInFile, boolean bLocRead, int indent)
        ...
        if (sLine != null) {
            while (sLine != null) {
                ...
                if (iType == KILL) {
                    ...
                    // Something broke, ignore it (the file we have been passed
                    // may be a standard logfile)
                    // System.out.println("Something broke");
                }
                sLine = brIn.readLine();
            }
    ...
    //
    //
    public void setIgnore(String[] aLocIgnore) {
        aIgnore = aLocIgnore;
    }
    ...
    private int getLineType(String sLine) {
        ...
        if ((sLine.length() >= TXTKILL.length()) && (sLine.substring(0, TXTKILL.length()).equalsIgnoreCase(TXTKILL))) {
            ...
            for (int iLoop = 0; iLoop < TXTSYSTEM.length; iLoop++) {
                if ((sLine.length() >= TXTSYSTEM[iLoop].length())
                        && (sLine.substring(0, TXTSYSTEM[iLoop].length()).equalsIgnoreCase(TXTSYSTEM[iLoop]))) {
                    iReturn = SYSTEM;
                    break;
                }
            }
    ...
    // Returns the string representation of the user who killed someone in the
    // passed in line
    //
    // Written by : Stuart Butcher
    //
    ...
    // Returns the string representation of the user who got killed in the
    // passed in line
    //
    // Written by : Stuart Butcher
    //
    ...
    //
    //
    public LogTotals getGrandTotal() {
        return grandTotal;
    }
    ...
    private void addKill(int iKiller, String sKiller, int iKilled, String sKilled, int iWeapon) {
        ...
        if ((!sKiller.equals(sKilled)) && (!sKiller.equals(TXTSUICIDE))) {
            ...
            if (htKills.get(IKiller) != null) {
                ...
                if (htHold.get(TXTTOTALS) != null) {
                    myTotal = (LogTotals) htHold.get(TXTTOTALS);
                }
                if (htHold.get(TXTOPONENTS) != null) {
                    htPeople = (Hashtable) htHold.get(TXTOPONENTS);
                    if (htPeople.get(IKilled) != null) {
                        myOponent = (LogTotals) htPeople.get(IKilled);
                    }
                }
            }
            ...
            if (htKills.get(IKilled) != null) {
                ...
                if (htHold.get(TXTOPONENTS) != null) {
                    htPeople = (Hashtable) htHold.get(TXTOPONENTS);
                    if (htPeople.get(IKiller) != null) {
                        myOponent = (LogTotals) htPeople.get(IKiller);
                    }
                }
            }
    ...
    // Processes a frag line by getting the killer, killed and weapon and
    // passing them to addKill
    //
    // Written by : Stuart Butcher
    //
    // Date : 12th September 2002
